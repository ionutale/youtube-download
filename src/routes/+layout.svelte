...
>
  ...
  <aside class="w-20 lg:w-64 flex-shrink-0 glass-panel border-r border-[var(--glass-border)] flex flex-col transition-all duration-300 z-20">
    ...
    <nav class="flex-1 py-6 flex flex-col gap-2 px-3">
      ...
      <a href="/history" class="flex items-center p-3 rounded-xl transition-all duration-200 hover:bg-[var(--glass-highlight)] {$page.url.pathname === '/history' ? 'bg-[var(--glass-highlight)] text-neon-blue shadow-lg shadow-neon-blue/10' : 'text-[var(--text-muted)] hover:text-[var(--text-color)]'}">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <span class="hidden lg:block ml-3 font-medium">History</span>
      </a>
